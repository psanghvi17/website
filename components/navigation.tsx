...
export default function Navigation() {
  ...
  return (
    <>
      ...
      {/* Sidebar Navigation */}
      <nav className={`fixed top-0 left-0 h-full z-50 bg-white/95 backdrop-blur-md border-r border-gray-200 shadow-lg transform transition-transform duration-300 ease-in-out ${
        isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
      } md:translate-x-0 md:relative md:z-auto md:bg-transparent md:border-r-0 md:shadow-none`}>
        
        <div className="flex flex-col h-full w-64 md:w-auto">
          {/* Header */}
          <div className="flex items-center justify-between p-4 md:p-0 md:mb-8">
            <Link 
              href="/" 
            ...
            <button
              onClick={closeMobileMenu}
              className="p-2 rounded-lg text-gray-700 hover:bg-gray-100 hover:text-blue-600 transition-colors duration-200 md:hidden"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
